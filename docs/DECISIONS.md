...
- 2026-07-27 — **Re-measured after pulling c723ec5/7246aac rather than taking the fix on trust: 4,096 concepts reachable from the seed (was 62), 0 orphaned beat nodes (was 293).** `REFLECT_MIN_CONCEPTS = 820` is 20.0% of 4,096, which is exactly what the constant's own comment claims it means. No change needed.
- 2026-07-27 — **The story payload moved to `public/story/` and is still imported, not fetched.** Making it async would push a load state through `starmap.ts`, `masking.ts`, `literacy.ts` and the whole render path — a real refactor, queued in BACKLOG rather than smuggled in beside a format change. The bundle cost is paid knowingly.
- 2026-07-27 — **Frame slots are filled in TWO places, deliberately.** Content fills `{here}` and any `{next}` it can, because only 446 of 4,096 concepts have a beat and therefore a label in the payload; the shell fills the rest from the ontology, where every concept has one. Content substitutes what content knows.
