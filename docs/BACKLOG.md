...
- [ ] **26 of the ticker's 38 word types can never be learned.** Carrier literacy is exposure across the beats the player has stood in, and `online`, `wore`, `loose`, `ahead`, `vocabulary`, `rot`, `solid`, `raw` and 18 more appear in ZERO beats, so they cannot reach `LEARN_AT` however far a player walks — measured against the shipped corpus, pinned by `test/ticker.test.ts` ("MEASURES THE CEILING"). Same open question as the HUD's four labels: a word that names a thing you DO cannot be taught by reading. Candidate rule, matching the readout one: exposure also counts the lines the player's own state proves were emitted (machines bought, milestones crossed) — derived, no save field.
